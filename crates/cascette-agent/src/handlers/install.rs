...
async fn install(
        ...
        && let Ok(others) = state.registry.find_by_install_path(path).await
    {
        ...
        if conflict {
            return Err(AGENT_ERROR_DIRECTORY_CONFLICT);
        }
    }
    ...
    match state.queue.insert(&operation).await {
        ...
        Err(e) => {
            ...
            if msg.contains("active") {
                Err(AGENT_ERROR_DUPLICATE_UID)
            } else {
                tracing::error!(error = %e, "CreateProductInstall Failed with error");
                Err(AGENT_ERROR_INVALID_REQUEST)
            }
        }
    }
}

pub async fn set_install(
    state: &Arc<AppState>,
    body: InstallRequest,
    product: String,
) -> Result<Value, u32> {
    let product_code = body.uid.as_deref().unwrap_or(&product);
    install(state, product_code, &body).await
}

pub async fn set_install_bare(state: &Arc<AppState>, body: InstallRequest) -> Result<Value, u32> {
    let product_code = match body.uid.as_deref() {
        Some(uid) if !uid.is_empty() => uid.to_string(),
        _ => return Err(AGENT_ERROR_INVALID_REQUEST),
    };
    install(state, &product_code, &body).await
}
