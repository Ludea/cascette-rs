...
pub async fn set_size_estimate(
    state: &Arc<AppState>,
    body: SizeEstimateRequest,
) -> Result<Value, u32> {
    ...
    if !state.size_cache.insert_pending(&uid).await {
        return Ok(json!({ "response_uri": format!("/size_estimate/{uid}") }));
    }
    ...
    Ok(json!({ "response_uri": format!("/size_estimate/{uid}") }))
}

/// GET /size_estimate/{uid} -- poll for estimation result.
pub async fn size_estimate_result(state: &Arc<AppState>, uid: String) -> Value {
    let response_uri = format!("/size_estimate/{uid}");

    let Some(entry) = state.size_cache.get(&uid).await else {
        return json!({ "response_uri": response_uri });
    };

    match entry.status {
        SizeEstimateStatus::Pending => json!({
            "uid": uid,
            "estimated_bytes": 0,
            "response_uri": response_uri,
        }),
        SizeEstimateStatus::Ready(bytes) => json!({
            "uid": uid,
            "estimated_bytes": bytes,
            "response_uri": response_uri,
        }),
        SizeEstimateStatus::Failed => json!({
            "uid": uid,
            "estimated_bytes": 0,
...
async fn estimate_size_background(state: &Arc<AppState>, uid: &str, encryption_key: Option<&str>) {
