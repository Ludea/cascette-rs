...
impl Default for CdnConfig {
    fn default() -> Self {
        Self {
            cdn_urls: vec![
                "http://level3.blizzard.com".to_string(),
                "http://eu.cdn.blizzard.com".to_string(),
                "http://us.cdn.blizzard.com".to_string(),
            ],
            max_retries: 3,
...
impl CdnClient {
    ...
    fn fetch_with_retry(&self, path: &str) -> NgdpCacheResult<Bytes> {
        // Update initial metrics
        {
            let mut metrics = self
                .metrics
                .write()
                .expect("CDN metrics lock should not be poisoned");
    ...
    /// Get CDN metrics
    pub fn metrics(&self) -> CdnMetrics {
        self.metrics
            .read()
            .expect("CDN metrics lock should not be poisoned")
            .clone()
...
mod tests {
    ...
    async fn test_cdn_backed_resolution_cache() {
        ...
        match result {
            ...
            Err(_) => {
                // Also acceptable - parse error from mock data
            }
            Ok(Some(_)) => {
