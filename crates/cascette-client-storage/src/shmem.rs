...
impl SharedMemoryManager {
    ...
    /// # Panics
    ///
    /// Panics if the message ID counter mutex is poisoned
    pub fn next_message_id(&self) -> u64 {
        let mut counter = self
            .message_id_counter
            .lock()
            .expect("message ID counter mutex should not be poisoned");
        let id = *counter;
...
mod windows {
    ...
    impl WindowsSharedMemory {
        /// Create a new Windows shared memory region
        #[allow(unsafe_code)]
    ...
    // Safety: WindowsSharedMemory is safe to send between threads
    #[allow(unsafe_code)]
    unsafe impl Send for WindowsSharedMemory {}
    // Safety: WindowsSharedMemory can be shared between threads with proper synchronization
    #[allow(unsafe_code)]
        ...
        fn drop(&mut self) {
            unsafe {
                ...
                let c_name = CString::new(self.name.clone())
                    .expect("shared memory name should be valid C string");
                shm_unlink(c_name.as_ptr());
            }
        }
    }

    // Safety: UnixSharedMemory is safe to send between threads
    #[allow(unsafe_code)]
    unsafe impl Send for UnixSharedMemory {}
    // Safety: UnixSharedMemory can be shared between threads with proper synchronization
    #[allow(unsafe_code)]
