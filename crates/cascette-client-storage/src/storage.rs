...
impl Storage {
    ...
    pub fn new(config: StorageConfig) -> Result<Self> {
        ...
        if !config.base_path.exists() {
            info!("Creating CASC storage directory: {:?}", config.base_path);
    ...
    fn validate_casc_directory_structure(base_path: &std::path::Path) -> Result<()> {
            ...
            if dir_path.exists() {
                ...
                if !dir_path.is_dir() {
                    return Err(crate::StorageError::Config(format!(
                        "{desc} path exists but is not a directory: {dir_path:?}"
                    )));
                }
                ...
                if let Err(e) = std::fs::write(&test_file, b"test") {
                    warn!("No write permission for {desc} directory {dir_path:?}: {e}");
                } else {
                    let _ = std::fs::remove_file(test_file); // Clean up test file
                }
            } else {
                info!("Creating CASC {desc} directory: {dir_path:?}");
        ...
        info!("CASC directory structure validated at: {base_path:?}");
        Ok(())
    }
