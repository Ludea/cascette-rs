...
pub fn compress_chunk(data: &[u8], mode: CompressionMode) -> BlteResult<Vec<u8>> {
    match mode {
        ...
        CompressionMode::LZ4 => {
            // LZ4HC compression with decompressed size prefix
            ...
            let max_compressed_size = lz4::block::compress_bound(data.len()).map_err(|e| {
                BlteError::CompressionError(format!("Failed to calculate LZ4 bound: {e}"))
            })?;
            ...
            let compressed_len =
                lz4::block::compress_to_buffer(data, None, false, &mut result[8..]).map_err(
                    |e| BlteError::CompressionError(format!("LZ4 compression failed: {e}")),
                )?;
...
pub fn decompress_chunk(data: &[u8], mode: CompressionMode) -> BlteResult<Vec<u8>> {
    match mode {
        ...
        CompressionMode::LZ4 => {
            ...
            // Convert size to i32 with bounds checking for LZ4 API
            let lz4_size_hint = i32::try_from(decompressed_size).map_err(|_| {
                BlteError::CompressionError("Decompressed size too large for LZ4".to_string())
            })?;

            let decompressed = lz4::block::decompress(compressed_data, Some(lz4_size_hint))
                .map_err(|e| {
...
mod tests {
    ...
    fn test_decrypt_chunk_salsa20() {
        ...
        // Create key store
        let key_store = TactKeyStore::new().expect("Should be able to create keyring store");
        let tact_key = TactKey::new(key_name, key);
        key_store.add(tact_key).expect("Should be able to add key");
...
pub fn decrypt_chunk_with_keys(
    ...
    let key = key_store
        .get(key_name)
        .map_err(|e| {
            BlteError::CompressionError(format!("Key lookup failed for 0x{key_name:016X}: {e}"))
        })?
        .ok_or_else(|| {
