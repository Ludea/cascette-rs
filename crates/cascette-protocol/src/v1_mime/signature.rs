...
fn verify_signatures(
    ...
    for (i, signer) in signers.iter().enumerate() {
        ...
        match verification_result {
            Ok(valid) => {
                if valid {
                    details.push(format!("Signer #{} verification: SUCCESS", i));
                } else {
                    details.push(format!("Signer #{} verification: FAILED", i));
                    all_valid = false;
                }
            }
            Err(e) => {
                details.push(format!("Signer #{} verification error: {}", i, e));
                all_valid = false;
            }
        }
    }
...
fn parse_rsa_public_key(key_bytes: &[u8]) -> Result<RsaPublicKey> {
    ...
    if let Ok(spki) = x509_cert::spki::SubjectPublicKeyInfoOwned::from_der(key_bytes) {
        if let Ok(key) = RsaPublicKey::from_pkcs1_der(spki.subject_public_key.raw_bytes()) {
            return Ok(key);
        }
    }
