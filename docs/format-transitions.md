...
The verification system includes intelligent caching to avoid repeated
downloads and organized file management.

### Cache Management

All downloaded files are cached in `tools/test_data/` with descriptive names:

- `{product}_{version}_{file_type}_{hash_prefix}.{extension}`

- Example: `wow_classic_era_1.13.7.38704_root_b98595f5.dat`

Cache management utilities:

```bash
# View cache statistics
python tools/clean_cache.py --list

# Clean specific file types
python tools/clean_cache.py --clean --type encoding --execute

# Clean all cache (dry run by default)
python tools/clean_cache.py --clean
```

#### Format Validation Tools

The following verification scripts validate our format understanding:

#### `tools/verify_root_transitions.py`

Tests Root file version detection across strategic builds:

```bash
python tools/verify_root_transitions.py
```

Expected outcomes:

- Classic Era: Root v3, TSFM magic

- Modern Retail: Root v3, TSFM magic

- Legacy detection for builds with Root v1/v2

#### `tools/verify_config_evolution.py`

Tracks build configuration field changes:

```bash
python tools/verify_config_evolution.py
```
...
- Value format changes (single vs space-separated)

- Cross-product consistency

#### `tools/verify_blte_evolution.py`

Analyzes BLTE compression usage patterns:

```bash
python tools/verify_blte_evolution.py
```
...
```bash
python tools/run_format_verification.py
```

Runs all verification scripts with summary reporting.
...
- [Format Evolution Analysis Tools](../tools/)
